...
const computeCost = (model, promptTokens, completionTokens) => {
    const price = priceLookup(model);
    if (!price) {
        if (model)
            console.warn("[ve-track][pricing] unknown model — cost falls back to 0", model);
        return 0;
    }
...
export const cloroCreditsToUsd = (credits) => Math.round(credits * CLORO_USD_PER_CREDIT * 1_000_000) / 1_000_000;
const CLORO_SYNC_ENDPOINTS = [
...
const readOpenAiUsage = async (resp) => {
...
const readAnthropicUsage = async (resp) => {
    if (isEventStream(resp)) {
        const chunks = parseSseData(await resp.text().catch(() => ""));
        let model = null;
        let input = 0;
...
export const PROVIDERS = [
    ...
    },
    {
        ...
        extract: async (resp) => {
            ...
            if (!payload)
                return null;
            const { usage, model } = payload;
            const totalPrompt = usage.promptTokenCount ?? usage.prompt_token_count ?? usage.promptTokens ?? 0;
            const cached = usage.cachedContentTokenCount ?? usage.cached_content_token_count ?? 0;
    ...
    },
    {
        ...
        extract: async (resp) => {
            ...
            const costUsd = typeof explicit === "number"
                ? explicit
    ...
    },
    {
        ...
        extract: async (resp) => {
            try {
                if (/\/async\/task\/?$/i.test(new URL(resp.url).pathname))
                    return null;
            }
            catch {
                /* */
            ...
            const model = typeof taskType === "string" ? taskType.toLowerCase() : (sync?.model ?? null);
