const EMPTY = { userId: null, orgId: null };
const headerOrgId = (req) => req.headers.get("x-organization-id") ??
    req.headers.get("X-Organization-Id");
export const clerkUserResolver = async (req, env) => {
    ...
    try {
        if (env.CLERK_SECRET_KEY && env.CLERK_PUBLISHABLE_KEY) {
            ...
            if (!userId)
                return EMPTY;
            const orgId = auth && "orgId" in auth && typeof auth.orgId === "string"
                ? auth.orgId
                : null;
