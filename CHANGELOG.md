...
- **Web docs parity** — the docs page now covers `maxExtractBytes`, lazy resolution, and the Clerk env var matrix (`CLERK_SECRET_KEY` / `CLERK_PUBLISHABLE_KEY` / `CLERK_JWT_KEY`), matching the README. The dashboard integrate snippet was corrected: it showed `trackedHandler` with a single merged object and a resolver returning a bare string; it now shows the canonical `trackHandler(config, handler)` quickstart.
